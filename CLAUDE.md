...
cargo test                                        # pager::tests (render SGR, wrap_ranges, clamp_pos, gutter, marks, files, follow, build_view, visible accessors, filtered info, parse_plus, parse_cless_env)
...
- **Default options (`CLESS` env var)** — like `less`'s `LESS`, the `CLESS` environment variable pre-sets startup options (`export CLESS="-N -S"`). `parse_cless_env` (pure, in `pager.rs`) splits it on whitespace (no shell quoting) and recognizes the same option tokens as the CLI (`-S`, `-N`, `-p pattern`, `+cmd`) — options only, no file paths. `main.rs` applies these before the argv loop, so command-line flags win (last-wins on the shared `wrap`/`numbers`/`start` state); there is no reverse flag to un-set a boolean from the CLI. Malformed tokens (unknown option, trailing `-p`, bad `+cmd`, bare word) are collected as short warnings and skipped rather than aborting; `main.rs` joins them as `CLESS: …` and passes them to the pager, which shows them on the status line (`message`) since stderr would be wiped by the alternate screen. A bad `CLESS` never blocks startup.
